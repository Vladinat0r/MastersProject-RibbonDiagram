...
Vector3d::Vector3d (SphericalCoords sph)
{
	setValues(sph);
}
...
SphericalCoords Vector3d::getSphericalRep()
{
	SphericalCoords result;

	result.rho = this->size();
	
	float tempS = sqrt(x*x+y*y);
	result.phi = acos(z / result.rho);
	
	if ( x >= 0) result.theata = asin(y/tempS);
	else result.theata = (float)(PI  - asin(y/tempS));

	return result;
}

void Vector3d::setValues(SphericalCoords spher)
{
	x = spher.rho * sin(spher.phi) * cos (spher.theata);
	y = spher.rho * sin(spher.phi) * sin (spher.theata);
	z = spher.rho * cos (spher.phi);
}
